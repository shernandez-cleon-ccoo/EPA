...
# Etiqueta legible "2026 T2" a partir de una fecha de fin de trimestre.
# OJO: R base NO tiene un especificador de trimestre en strftime/format()
# ("%q" no existe en R base — sí en el paquete zoo, pero no lo cargamos
# aquí — usar format(fecha, "%Y T%q") deja el "%q" literal sin sustituir
# y produce "2026 Tq" en vez de "2026 T2"). Se calcula el trimestre a
# mano a partir del mes.
formato_trimestre <- function(fecha) {
  mes <- as.integer(format(fecha, "%m"))
  trimestre <- (mes - 1) %/% 3 + 1
  sprintf("%s T%d", format(fecha, "%Y"), trimestre)
}
